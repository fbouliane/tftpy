...
class TftpPacketInitial(TftpPacket, TftpPacketWithOptions):
    ...
    def encode(self):
        ...
        if self.mode == "octet":
            ...
            raise AssertionError("Unsupported mode: {}".format(self.mode))
        # Add options.
        options_list = []
        if len(self.options) > 0:
            ...
            for key in self.options:
                ...
                format += "%dsx" % len(key)
                options_list.append(str.encode(key))
    ...
    def decode(self):
        ...
        self.mode = mystruct[1].decode().lower() # force lc - bug 17
...
::
    ...
    def encode(self):
        ...
        format = "!HH{}s".format(len(self.data))
        self.buffer = struct.pack(format,
                                  self.opcode,
                                  self.blocknumber,

...
class TftpStateServerStart(TftpState):
    ...
    def handle(self, pkt, raddress, rport):
        ...
        if isinstance(pkt, TftpPacketRRQ):
            ...
            self.sendError(TftpErrors.IllegalTftpOp)
            raise TftpException("Invalid packet to begin up/download: {}".format(pkt))
...
class TftpStateExpectACK(TftpState):
    ...
    def handle(self, pkt, raddress, rport):
        ...
        if isinstance(pkt, TftpPacketACK):
            ...
            raise TftpException("Received ERR packet from peer: {}".format(str(pkt)))
        else:
